...
public:
    AirportCounter() {
        airportIdToDeparturesCount.fill(0);
    }
    ...
    Items GetItems() const {
        ...
        for (size_t airport_idx = 0; airport_idx < SIZE; ++airport_idx) {
            items[airport_idx] = {static_cast<TAirport>(airport_idx), airportIdToDeparturesCount[airport_idx]};
        }
        return items;
    }
...
private:
    size_t& GetRef(TAirport airport) {
        return airportIdToDeparturesCount[static_cast<size_t>(airport)];
    }

    const size_t& GetRef(TAirport airport) const {
        return airportIdToDeparturesCount[static_cast<size_t>(airport)];
    }

    array<size_t, SIZE> airportIdToDeparturesCount;
};
