...
SearchServer::SearchServer(istream &document_input) : isFirstUpdate(true) { UpdateDocumentBase(document_input); }

void SearchServer::UpdateDocumentBase(istream &document_input) { UpdateDocumentBaseSync(document_input); }
...
void SearchServer::AddQueriesStream(istream &query_input, ostream &search_results_output) {
  if (futures.size() >= MAX_THREADS) {
    while (true) {
      for (auto &fut : futures) {
        if (fut.wait_for(1ms) == future_status::ready) {
          fut = async(&SearchServer::AddQueriesStreamSync, this, ref(query_input), ref(search_results_output));
          return;
        }
      }
    ...
    futures.push_back(async(&SearchServer::AddQueriesStreamSync, this, ref(query_input), ref(search_results_output)));
  }
}
...
void SearchServer::AddQueriesStreamSync(istream &query_input, ostream &search_results_output) {
  ...
  for (string current_query; getline(query_input, current_query);) {
    ...
    partial_sort(begin(docId_count), begin(docId_count) + static_cast<long>(min(5ul, docId_count.size())),
                 end(docId_count), [](auto &lhs, auto &rhs) {
