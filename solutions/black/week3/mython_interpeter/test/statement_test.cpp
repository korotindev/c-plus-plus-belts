...
#define ASSERT_OBJECT_VALUE_EQUAL(obj, expected)                                              \
  ...
  void TestBadAddition() {
    ...
    ASSERT_THROWS(Add(make_unique<NumericConst>(42), make_unique<StringConst>("4"s)).Execute(empty),
                  std::runtime_error);
    ASSERT_THROWS(Add(make_unique<StringConst>("4"s), make_unique<NumericConst>(42)).Execute(empty),
                  std::runtime_error);
