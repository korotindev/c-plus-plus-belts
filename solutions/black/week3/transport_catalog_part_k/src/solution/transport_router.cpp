...
void TransportRouter::FillGraphWithStops(shared_ptr<const TransportInfo> transport_info) {
  ...
  for (const auto stop_ptr : transport_info->GetStopsRange()) {
    auto& vertex_ids = stops_vertex_ids_[stop_ptr->name];
    vertex_ids.in = vertex_id++;
    vertex_ids.out = vertex_id++;
    vertices_info_[vertex_ids.in] = {stop_ptr->name};
    vertices_info_[vertex_ids.out] = {stop_ptr->name};
...
void TransportRouter::FillGraphWithBuses(shared_ptr<const TransportInfo> transport_info) {
  for (const auto bus_ptr : transport_info->GetBusesRange()) {
    const size_t stop_count = bus_ptr->stops.size();
    if (stop_count <= 1) {
      continue;
    }

    for (size_t start_stop_idx = 0; start_stop_idx + 1 < stop_count; ++start_stop_idx) {
      const Graph::VertexId start_vertex = stops_vertex_ids_[bus_ptr->stops[start_stop_idx]].in;
      int total_distance = 0;
      for (size_t finish_stop_idx = start_stop_idx + 1; finish_stop_idx < stop_count; ++finish_stop_idx) {
        total_distance +=
            transport_info->ComputeStopsDistance(bus_ptr->stops[finish_stop_idx - 1], bus_ptr->stops[finish_stop_idx]);
        edges_info_.push_back(BusEdgeInfo{
            .bus_name = bus_ptr->name,
            .span_count = finish_stop_idx - start_stop_idx,
        });
        const Graph::EdgeId edge_id = graph_.AddEdge({
            start_vertex, stops_vertex_ids_[bus_ptr->stops[finish_stop_idx]].out,
