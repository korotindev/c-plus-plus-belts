...
namespace Graph {
  ...
  std::optional<typename Router<Weight>::RouteInfo> Router<Weight>::BuildRoute(VertexId from, VertexId to) const {
    ...
    routes_internal_data_[from] = RouteInternalData{.weight = 0, .prev_edge = {}};
    ...
    while (!vertices_by_weight.empty()) {
      const auto min_vertex = *vertices_by_weight.begin();
      ...
      for (const EdgeId edge_id : graph_.GetIncidentEdges(min_vertex.second)) {
        ...
        if (!routes_internal_data_[edge.to] ||
            routes_internal_data_[edge.to]->weight > min_vertex.first + edge.weight) {
          if (routes_internal_data_[edge.to]) {
            vertices_by_weight.erase({routes_internal_data_[edge.to]->weight, edge.to});
          }
          routes_internal_data_[edge.to] =
              RouteInternalData{.weight = min_vertex.first + edge.weight, .prev_edge = edge_id};
