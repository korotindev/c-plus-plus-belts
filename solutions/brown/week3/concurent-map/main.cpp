...
public:
  ...
  WriteAccess operator[](const K &key) {
    size_t bucket_index = hasher(key) % bucket_count;
    return {lock_guard(mutexes[bucket_index]), maps[bucket_index][key]};
  }

  ReadAccess At(const K &key) const {
    size_t bucket_index = hasher(key) % bucket_count;
    return {lock_guard(mutexes[bucket_index]), maps[bucket_index].at(key)};
  }

  bool Has(const K &key) const {
    size_t bucket_index = hasher(key) % bucket_count;
    lock_guard<mutex> g(mutexes[bucket_index]);
    return maps[bucket_index].count(key) > 0;
  }
