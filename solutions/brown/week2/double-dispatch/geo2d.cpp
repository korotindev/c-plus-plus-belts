...
bool Collide(Point p, Circle c) { return DistanceSquared(p, c.center) <= Sqr<uint64_t>(c.radius); }
...
bool Collide(Rectangle r, Segment s) {
  return Collide(s.p1, r) || Collide(s.p2, r) || Collide(s, Segment{r.BottomLeft(), r.BottomRight()}) ||
         Collide(s, Segment{r.BottomRight(), r.TopRight()}) || Collide(s, Segment{r.TopRight(), r.TopLeft()}) ||
...
bool Collide(Rectangle r, Circle c) {
  return Collide(c.center, r) || Collide(c, Segment{r.BottomLeft(), r.BottomRight()}) ||
         Collide(c, Segment{r.BottomRight(), r.TopRight()}) || Collide(c, Segment{r.TopRight(), r.TopLeft()}) ||
