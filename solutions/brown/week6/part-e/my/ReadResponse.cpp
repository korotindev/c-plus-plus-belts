...
ReadBusResponse::ReadBusResponse(string busName, size_t requestId) : ReadResponse(requestId), busName(move(busName)) {}

Json::Document ReadNoBusResponse::ToJson() {
  return Json::Document(map<string, Json::Node>({
...
ReadStopResponse::ReadStopResponse(string stopName, size_t requestId)
    : ReadResponse(requestId), stopName(std::move(stopName)) {}
...
Json::Document ReadStopMetricsResponse::ToJson() {
  return Json::Document(
      map<string, Json::Node>({{"buses", vector<Json::Node>(buses.begin(), buses.end())}, {"request_id", requestId}}));
}
...
ReadRouteResponseWaitItem::ReadRouteResponseWaitItem(std::string_view stopName_, double time_)
    : ReadRouteResponseItem(ReadRouteResponseItemType::WaitType, time_), stopName(stopName_) {}

Json::Node ReadRouteResponseWaitItem::ToJson() const {
  return Json::Node(
      map<string, Json::Node>({{"type", string("Wait")}, {"stop_name", string(stopName)}, {"time", time}}));
}

Json::Node ReadRouteResponseBusItem::ToJson() const {
  return Json::Node(map<string, Json::Node>(
      {{"type", string("Bus")}, {"bus", string(busName)}, {"span_count", spanCount}, {"time", time}}));
}

ReadRouteResponseBusItem::ReadRouteResponseBusItem(std::string_view busName_, double time_, size_t spanCount_)
    : ReadRouteResponseItem(ReadRouteResponseItemType::BusType, time_), busName(busName_), spanCount(spanCount_) {}
