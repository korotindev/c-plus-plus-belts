...
Node *FindHighestVisitedParent(Node *me) {
  auto pointer = me;
  while (pointer && pointer->parent && pointer->parent->right == pointer) {
...
Node *Next(Node *me) {
  ...
  if (me->right) {
    return FindLeftMost(me->right);
  }

  auto pointer = FindHighestVisitedParent(me);
