...
void PrintStats(vector<Person> persons) {
  auto females_point = partition(begin(persons), end(persons), [](const Person& p) {
    return p.gender == Gender::MALE;
  });

  auto unemployed_males_point = partition(begin(persons), females_point, [](const Person& p) {
    return p.is_employed;
  });

  auto unemployed_females_point = partition(females_point, end(persons), [](const Person& p) {
    return p.is_employed;
  });
