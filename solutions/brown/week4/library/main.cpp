...
struct Library {
  ...
  explicit Library(vector<string> a_book_names, IBooksUnpacker &unpacker) : book_names(std::move(a_book_names)) {
  ...
  for (int task_num = 0; task_num < tasks_count; ++task_num) {
    tasks.push_back(async([&cache, &lib, task_num] {
      ...
      for (int i = 0; i < trials_count; ++i) {
        ...
        ASSERT_EQUAL(cache->GetBook(book_name)->GetContent(), lib.content.find(book_name)->second->GetContent());
      }
      stringstream ss;
