    ...
    namespace RealSphere {
      ...
      class RealProjector {
       public:
        ...
        RealProjector(PointInputIt points_begin, PointInputIt points_end, double max_width, double max_height,
                      double padding);

        Svg::Point operator()(Sphere::Point point) const;
      ...
      template <typename PointInputIt>
      RealProjector::RealProjector(PointInputIt points_begin, PointInputIt points_end, double max_width,
        ...
        const auto [left_it, right_it] =
            std::minmax_element(points_begin, points_end,
                                [](Sphere::Point lhs, Sphere::Point rhs) { return lhs.longitude < rhs.longitude; });
        min_lon_ = left_it->longitude;
        const double max_lon = right_it->longitude;

        const auto [bottom_it, top_it] =
            std::minmax_element(points_begin, points_end,
                                [](Sphere::Point lhs, Sphere::Point rhs) { return lhs.latitude < rhs.latitude; });
