  ...
  for (const auto& [_, bus_ptr] : buses_dict) {
    ...
    for (size_t stop_idx = 1; stop_idx < stops.size(); ++stop_idx) {
      if (support_stops.count(stops[stop_idx])) {
        ...
        const double lat_step = (next_coord.latitude - prev_coord.latitude) / static_cast<double>(stop_idx - last_support_idx);
        const double lon_step = (next_coord.longitude - prev_coord.longitude) / static_cast<double>(stop_idx - last_support_idx);
...
public:
  ...
  double MapLat(double source) const {
    return Find(lats_, source).target;
  }
  double MapLon(double source) const {
    return Find(lons_, source).target;
  }
