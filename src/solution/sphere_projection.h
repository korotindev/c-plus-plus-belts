...
namespace Sphere {
  ...
  class Projector {
   public:
    ...
    Projector(PointInputIt points_begin, PointInputIt points_end, double max_width, double max_height, double padding);
  ...
  template <typename PointInputIt>
  Projector::Projector(PointInputIt points_begin, PointInputIt points_end, double max_width, double max_height,
                       double padding)
      : padding_(padding) {
    ...
    const auto [left_it, right_it] = std::minmax_element(
        points_begin, points_end, [](Point lhs, Point rhs) { return lhs.longitude < rhs.longitude; });
    min_lon_ = left_it->longitude;
    const double max_lon = right_it->longitude;

    const auto [bottom_it, top_it] =
        std::minmax_element(points_begin, points_end, [](Point lhs, Point rhs) { return lhs.latitude < rhs.latitude; });
